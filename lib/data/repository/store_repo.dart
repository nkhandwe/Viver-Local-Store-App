...
class StoreRepo {
  ...
  Future<Response> getItemList(String offset, String type) async {
    return await apiClient.getData(
        '${AppConstants.ITEM_LIST_URI}?offset=$offset&limit=10&type=$type');
  }
  ...
  Future<Response> updateStore(Store store, XFile logo, XFile cover, String min,
      String max, String type) async {
    ...
    return apiClient.postMultipartData(
      AppConstants.VENDOR_UPDATE_URI,
      _fields,
  ...
  Future<Response> addItem(
    ...
    if (Get.find<SplashController>().configModel.moduleConfig.module.stock) {
      _fields
          .addAll((<String, String>{'current_stock': item.stock.toString()}));
    }
    ...
    for (int index = 0; index < item.addOns.length; index++) {
      _addon = _addon +
          '${index == 0 ? item.addOns[index].id : ',${item.addOns[index].id}'}';
    }
    _fields.addAll(<String, String>{'addon_ids': _addon});
    if (item.categoryIds.length > 1) {
      _fields
          .addAll(<String, String>{'sub_category_id': item.categoryIds[1].id});
    }
    if (!isAdd) {
      _fields.addAll(<String, String>{
        '_method': 'put',
    ...
    return apiClient.postMultipartData(
      isAdd ? AppConstants.ADD_ITEM_URI : AppConstants.UPDATE_ITEM_URI,
      _fields,
      _images,
    );
  }
  ...
  Future<Response> deleteItem(int itemID) async {
    return await apiClient
        .deleteData('${AppConstants.DELETE_ITEM_URI}?id=$itemID');
  }

  Future<Response> getStoreReviewList(int storeID) async {
    return await apiClient
        .getData('${AppConstants.VENDOR_REVIEW_URI}?store_id=$storeID');
  }
  ...
  Future<Response> updateItemStatus(int itemID, int status) async {
    return await apiClient.getData(
        '${AppConstants.UPDATE_ITEM_STATUS_URI}?id=$itemID&status=$status');
  }

  Future<Response> addSchedule(Schedules schedule) async {
    return await apiClient.postData(
        AppConstants.ADD_SCHEDULE, schedule.toJson());
  }

  Future<Response> deleteSchedule(int scheduleID) async {
    return await apiClient
        .deleteData('${AppConstants.DELETE_SCHEDULE}$scheduleID');
  }
  ...
  Future<Response> getGlobalItemList(String offset, String type) async {
    return await apiClient.getData(
        '${AppConstants.GLOBAL_ITEMS_URI}?offset=$offset&limit=10&type=$type');
  }

  Future<Response> addGlobalItems(List<int> items) async {
    return await apiClient
        .postData(AppConstants.STORE_GLOBAL_ITEMS_URI, {'item_ids': items});
  }
}
