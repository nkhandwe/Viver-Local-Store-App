...
class StoreController extends GetxController implements GetxService {
  ...
  List<Item> _itemList;
  List<Item> _globalItemList;
  List<int> _selectedGlobalItemList;
  ...
  List<Item> get globalItemList => _globalItemList;
  List<int> get selectedGlobalItemList => _selectedGlobalItemList;
  List<ReviewModel> get storeReviewList => _storeReviewList;
  List<ReviewModel> get itemReviewList => _itemReviewList;
  bool get isLoading => _isLoading;
  int get pageSize => _pageSize;
  int get globalPageSize => _globalPageSize;
      ...
      if (item != null && item.addOns != null) {
        for (int index = 0; index < item.addOns.length; index++) {
          setSelectedAddonIndex(
              _addonsIds.indexOf(item.addOns[index].id), false);
        }
      }
    }
  ...
  Future<void> updateStore(
    ...
    update();
    Response response =
        await storeRepo.updateStore(store, _rawLogo, _rawCover, min, max, type);
    if (response.statusCode == 200) {
  ...
  Future<void> addItem(Item item, bool isAdd) async {
      ...
      _attributeList.forEach((attributeModel) {
        if (attributeModel.active) {
          ...
          attributeModel.variants.forEach((variant) {
            _variantString = _variantString +
                '${_variantString.isEmpty ? '' : ','}' +
                variant.replaceAll(' ', '');
          });
          _fields.addAll(<String, String>{
            'choice_options_${attributeModel.attribute.id}':
                jsonEncode([_variantString])
          });
        }
      });
      _fields.addAll(<String, String>{
        'attribute_id': jsonEncode(_idList),
        'choice_no': jsonEncode(_idList),
        'choice': jsonEncode(_nameList)
      });
    ...
    Response response = await storeRepo.addItem(
        item, _rawLogo, _rawImages, _savedImages, _fields, isAdd);
    if (response.statusCode == 200) {
    ...
    for (int i = 0; i < _length; i++) {
      ...
      if (item != null) {
        ...
        _variantTypeList.add(VariantTypeModel(
          variantType: _value,
          priceController:
              TextEditingController(text: _price > 0 ? _price.toString() : ''),
          priceNode: FocusNode(),
          stockController:
              TextEditingController(text: _stock > 0 ? _stock.toString() : ''),
          stockNode: FocusNode(),
        ));
      } else {
        _variantTypeList.add(VariantTypeModel(
  ...
  void setTotalStock() {
    _totalStock = 0;
    _variantTypeList.forEach((variant) => _totalStock =
        variant.stockController.text.trim().isNotEmpty
  ...
  Future<void> getGlobalItemList(String offset, String type,
      {bool notify = true}) async {
    if (offset == '1') {
      _globalOffsetList = [];
      _globalOffset = 1;
      _type = type;
      _globalItemList = null;
      _selectedGlobalItemList = [];
      if (notify) {
        update();
      }
    }
    if (!_globalOffsetList.contains(offset)) {
      _globalOffsetList.add(offset);
      Response response = await storeRepo.getGlobalItemList(offset, type);
      if (response.statusCode == 200) {
        if (offset == '1') {
          _globalItemList = [];
        }
        _globalItemList.addAll(ItemModel.fromJson(response.body).items);
        _globalPageSize = ItemModel.fromJson(response.body).totalSize;
        _isLoading = false;
        update();
      } else {
        ApiChecker.checkApi(response);
  ...
  void setGlobalOffset(int offset) {
    _globalOffset = offset;
  }

  void toggleGlobalSelection(int index) {
    if (_selectedGlobalItemList.contains(index)) {
      _selectedGlobalItemList.removeAt(_selectedGlobalItemList.indexOf(index));
    } else {
      _selectedGlobalItemList.add(index);
    }
    update();
  }

  Future<void> addGlobalItems(List<int> items) async {
    _scheduleLoading = true;
    update();
    Response response = await storeRepo.addGlobalItems(items);
    if (response.statusCode == 200) {
      Get.back();
      showCustomSnackBar('items_added_successfully'.tr, isError: false);
    } else {
      ApiChecker.checkApi(response);
    }
    _scheduleLoading = false;
